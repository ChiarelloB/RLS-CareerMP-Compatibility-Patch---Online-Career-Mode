...
local multiplayerSafeRepairTime = 2

local function isMultiplayerRepairSafeMode()
  return MPCoreNetwork and MPCoreNetwork.isMPSession and MPCoreNetwork.isMPSession()
end

local function getMultiplayerSafeRepairTime(repairTime)
  if type(repairTime) == "number" and repairTime <= 0 then
    return multiplayerSafeRepairTime
  end
  return repairTime
end

local function getRepairTimeChoiceText(repairTime)
  local displayRepairTime = getMultiplayerSafeRepairTime(repairTime)
  if type(displayRepairTime) ~= "number" or displayRepairTime <= 0 then
    return "Instant"
  end
  if displayRepairTime < 60 or displayRepairTime % 60 ~= 0 then
    return string.format("%i sec", displayRepairTime)
  end
  return string.format("%i min", displayRepairTime / 60)
end
...
local function loadInsurancesData(resetSomeData)
  ...
  for _, insuranceProviderFileName in ipairs(insuranceProvidersFileNames) do
    ...
    for coverageOptionName, coverageOptionInfo in pairs(insuranceInfo.coverageOptions.repairTime.choices) do
      coverageOptionInfo.choiceText = getRepairTimeChoiceText(coverageOptionInfo.value)
    end
    for coverageOptionName, coverageOptionInfo in pairs(insuranceInfo.coverageOptions.deductible.choices) do
...
local function startRepairDelayed(vehInfo, repairTime)
  if career_modules_inventory.getVehicleIdFromInventoryId(vehInfo.id) then -- vehicle is currently spawned
    if vehInfo.id == career_modules_inventory.getCurrentVehicle() then
      startRepairVehInfo = {vehId = vehInfo.id, inventoryId = vehInfo.id, repairTime = repairTime}
...
local function startRepairMultiplayerSafe(vehInfo, repairTime, callback)
  local spawnedVehId = career_modules_inventory.getVehicleIdFromInventoryId(vehInfo.id)
  local wasCurrentVehicle = vehInfo.id == career_modules_inventory.getCurrentVehicle()

  if spawnedVehId then
    career_modules_inventory.removeVehicleObject(vehInfo.id)
  end

  if repairTime and repairTime > multiplayerSafeRepairTime then
    career_modules_inventory.delayVehicleAccess(vehInfo.id, repairTime, "repair")
    onAfterVehicleRepaired(vehInfo)
    return
  end
  ...
  local function finishRepair()
    career_modules_inventory.spawnVehicle(vehInfo.id, nil, function()
      if wasCurrentVehicle then
        career_modules_inventory.enterVehicle(vehInfo.id)
      end
      if callback then
        callback(vehInfo)
      end
  ...
  if repairTime and repairTime > 0 then
    core_jobsystem.create(function(job)
      job.sleep(repairTime)
      finishRepair()
    end)
  else
    finishRepair()
...
local function startRepair(vehInvId, repairOptionData, callback)
  ...
  repairOptionData.repairTime = getMultiplayerSafeRepairTime(repairOptionData.repairTime)
  ...
  if callback and career_modules_inventory.getVehicleIdFromInventoryId(vehInfo.id) then
    startRepairMultiplayerSafe(vehInfo, repairOptionData.repairTime or 0, callback)
    return
  end
...
local function startRepairInGarage(invVehId, repairOptionData)
  ...
  return startRepair(invVehId, repairOptionData,
    function(vehInfo)
      local activeVehId = career_modules_inventory.getVehicleIdFromInventoryId(invVehId)
      local vehObj = activeVehId and getObjectByID(activeVehId)
...
local function onEnterVehicleFinished(invVehId)
  if startRepairVehInfo then
    local delayedInvVehId = startRepairVehInfo.inventoryId or startRepairVehInfo.vehId or invVehId
    local vehInfo = delayedInvVehId and career_modules_inventory.getVehicles()[delayedInvVehId]
    if vehInfo then
      startRepairDelayed(vehInfo, startRepairVehInfo.repairTime)
    end
...
M.getMultiplayerSafeRepairTime = getMultiplayerSafeRepairTime
M.getRepairTimeChoiceText = getRepairTimeChoiceText
