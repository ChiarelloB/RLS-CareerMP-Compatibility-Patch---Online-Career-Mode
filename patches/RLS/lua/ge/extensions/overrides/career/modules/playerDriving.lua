...
local M = {}
local compat = require("lua.ge.extensions.overrides.career.modules.careermpCompat")
...
local function getCareerMPClientConfig()
  if not extensions or not extensions.careerMPEnabler or not extensions.careerMPEnabler.getClientConfig then
    return nil
  end
  return extensions.careerMPEnabler.getClientConfig()
end

local function installCompatPatches()
  compat.install()
end
...
local function setupTraffic(forceSetup)
  ...
  if forceSetup or
    ...
    local clientConfig = getCareerMPClientConfig()
    local roadTrafficDisabled = clientConfig and clientConfig.roadTrafficEnabled == false
    local parkedTrafficDisabled = clientConfig and clientConfig.parkedTrafficEnabled == false
    ...
    if roadTrafficDisabled then
      amount = 0
    elseif amount == 0 then -- auto amount
      amount = gameplay_traffic.getIdealSpawnAmount()
    end
    if not getAllVehiclesByType()[1] and amount > 0 then -- if player vehicle does not exist yet
      amount = amount - 1
    end
    amount = math.max(0, amount)
    if not M.debugMode and amount > 0 then
    ...
    if isNoParkedModActive() or parkedTrafficDisabled then
      parkedAmount = 0
    else
      if parkedAmount == 0 then -- auto amount
        parkedAmount = clamp(gameplay_traffic.getIdealSpawnAmount(nil, true), 4, 20)
      end
      parkedAmount = math.max(0, parkedAmount)
      if not M.debugMode and parkedAmount > 0 then
    ...
    if roadTrafficDisabled or isNoPoliceModActive() then
      policeAmount = 0
    end
    ...
    if playerData.trafficActive == 0 and not roadTrafficDisabled then
