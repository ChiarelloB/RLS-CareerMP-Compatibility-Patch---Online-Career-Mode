...
-- Format a group of identical parcels as a prop cargo card that can be spawned
-- directly into the world instead of loaded into a vehicle container.
local function formatCargoGroupAsProp(group, showFirstSeen)
  ...
  local ret = {
    ...
    rewardMoney = group[1].rewards and (group[1].rewards.money * 2) or 0,
    weight = group[1].weight,
    enabled = true,
    disableReason = {},
    targetLocations = {},
  ...
  if showFirstSeen and group[1].firstSeen and (dGeneral.time() - group[1].firstSeen) < 3 then
    ret.showNewTimer = 3 - (dGeneral.time() - group[1].firstSeen)
  end
      ...
      if mod.type == "timed" then
        ...
        if group[1].loadedAtTimeStamp then
          if expiredTime <= delayedTime then
            ret.remainingTime = {
              type = "untilDelayed",
              time = delayedTime - expiredTime,
              percent = (delayedTime - expiredTime) / delayedTime
            }
          elseif expiredTime <= lateTime then
            ret.remainingTime = {
              type = "untilLate",
              time = lateTime - expiredTime,
              percent = (lateTime - expiredTime) / (lateTime - delayedTime)
            }
          else
            ret.remainingTime = {
              type = "late",
  ...
  table.insert(ret.targetLocations, {
    type = "propDelivery",
    label = "Spawn as Prop",
    enabled = true,
    maxAmount = #group,
  ...
  for _ = 1, #group - transientMoveCounts do
...
local function requestCargoDataForUi(facId, psPath, updateMaxTimeTimestamp)
  ...
  dGeneral.getNearbyVehicleCargoContainers(function(playerCargoContainers)
      ...
      facilityPanels = {
        {
          ...
          filterValueButtons = {'parcel','propCargo','trailer','material','vehicle'},
          heading = "Logistics",
    ...
    -- Inject player cards for active prop cargo deliveries. These live in the
    -- parcel manager but not in a physical vehicle container, so they need to
    -- be added manually for the UI and confirm flow.
      ...
      for _, task in ipairs(career_modules_delivery_propCargo.getPropTasks()) do
        ...
        if #propCargoGroup > 0 then
          ...
          local card = {
            ...
            bigMapIds = {
              [string.format("delivery-parking-%s-%s", first.destination.facId, first.destination.psPath)] = true
            },
            route = {
              type = "navgraph",
          ...
          addCard(card)
          uiData.availableSystems = uiData.availableSystems or {}
          uiData.availableSystems.parcelDelivery = true
        end
      end
    end
