...
local function resetRacerCompatTiming(racer)
  racer.rlsCareerMPDragPreviousDistance = nil
end

local function getCompatPreviousDistance(racer, distanceFromOrigin)
  return racer.rlsCareerMPDragPreviousDistance or racer.previousDistanceFromOrigin or distanceFromOrigin
end

local function getOnlineRaceTimerDelta(racer, timers, distanceFromOrigin, prevDistance, dtSim)
  if not isMultiplayerSession() or not timers.reactionTime or not timers.reactionTime.isSet then
    return dtSim
  end

  if not distanceFromOrigin or not prevDistance then
    return dtSim
  end

  local distanceDelta = distanceFromOrigin - prevDistance
  if distanceDelta <= 0.0001 then
    -- BeamMP can make the drag timer update more than once for the same
    -- distance sample. Once the launch beam has been crossed, do not count a
    -- duplicated sample as extra elapsed ET.
    return 0
  end

  local currentSpeed = racer.vehSpeed or 0
  local previousSpeed = racer.prevSpeed or currentSpeed
  local averageSpeed = (currentSpeed + previousSpeed) * 0.5
  if averageSpeed <= 0.1 then
    return dtSim
  end

  local distanceBasedDelta = distanceDelta / averageSpeed
  if distanceBasedDelta > 0 and distanceBasedDelta < dtSim then
...
local function onUpdate(dtReal, dtSim, dtRaw)
  ...
  for _, racer in pairs(dragData.racers) do
    ...
    if gameplay_drag_utils then
      ...
      local prevDistance = getCompatPreviousDistance(racer, distanceFromOrigin)
      local timerDelta = getOnlineRaceTimerDelta(racer, timers, distanceFromOrigin, prevDistance, dtSim)
      timerValue = timerValue + timerDelta -- dtsim is 0.01
      ...
      for timerName, timer in pairs(timers) do
        if addFrameHistoryDebug[timerName] and not timer.isSet and timer.type == "distanceTimer" then
          ...
          timer.frameHistory[#timer.frameHistory+1] = string.format("Racer: %d Frame %d: %0.5fs after start. Distance: %0.4fm (before: %0.4fm). Frame duration: %0.5fs", racer.vehId, #timer.frameHistory+1, timerValue, distanceFromOrigin, prevDistance, timerDelta)
        end
        ...
        if timer.distance and not timer.isSet and distanceFromOrigin >= timer.distance then
          ...
          if timer.type == "distanceTimer" then
            timer.value = timerValue - (1 - t) * timerDelta
            timer.isSet = true
      ...
      racer.rlsCareerMPDragPreviousDistance = distanceFromOrigin
